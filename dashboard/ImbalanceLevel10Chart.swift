...
struct ImbalanceLevel10Chart: View {
    @EnvironmentObject var book: OrderBookData
    
    func getBidVolume(_ stats: Stats, _ prevStats: Stats!) -> Double {
        if let pr = prevStats {
            if stats.bestBid > pr.bestBid {
                return stats.bestBidVolume + stats.totalBidVol1_10
            } else if stats.bestBid == pr.bestBid {
                return stats.bestBidVolume - pr.bestBidVolume + stats.totalBidVol1_10
            } else if stats.bestBid < pr.bestBid {
                return -1 * (pr.bestBidVolume + pr.totalBidVol1_10)
            }
        }
    ...
    func getAskVolume(_ stats: Stats, _ prevStats: Stats!) -> Double {
        if let pr = prevStats {
            if stats.bestAsk < pr.bestAsk {
                return stats.bestAskVolume + stats.totalAskVol1_10
            } else if stats.bestAsk == pr.bestAsk {
                return stats.bestAskVolume - pr.bestAskVolume  + stats.totalAskVol1_10
            } else if stats.bestAsk > pr.bestAsk {
                return -1 * (pr.bestAskVolume + pr.totalAskVol1_10)
            }
        }
